...
class WC_Pakettikauppa {
  ...
  public function display_error() {
    wc_add_notice(__('An error occured. Please try again later.', WC_PAKETTIKAUPPA_TEXT_DOMAIN), 'error');
  }
  ...
  public function pickup_point_field_html() {
    ...
    echo '<th>' . esc_attr__('Pickup point', WC_PAKETTIKAUPPA_TEXT_DOMAIN) . '</th>';
    echo '<td data-title="' . esc_attr__('Pickup point', WC_PAKETTIKAUPPA_TEXT_DOMAIN) . '">';
    ...
    if ( empty($shipping_postcode) ) {
      echo '<p>';
      esc_attr_e('Insert your shipping details to view nearby pickup points', WC_PAKETTIKAUPPA_TEXT_DOMAIN);
      echo '</p>';
    } elseif ( ! is_numeric($shipping_postcode) ) {
      echo '<p>';
      printf(
        /* translators: %s: Postcode */
        esc_attr__('Invalid postcode "%1$s". Please check your address information.', WC_PAKETTIKAUPPA_TEXT_DOMAIN),
      ...
      if ( $options_array !== false ) {
        ...
        printf(
          /* translators: %s: Postcode */
          esc_html__('Choose one of the pickup points close to your postcode %1$s below:', WC_PAKETTIKAUPPA_TEXT_DOMAIN),
  ...
  private function process_pickup_points_to_option_array( $pickup_point_data ) {
    ...
    $options_array = array( '__NULL__' => '- ' . __('Select a pickup point', WC_PAKETTIKAUPPA_TEXT_DOMAIN) . ' -' );
  ...
  public function display_order_data( $order ) {
    ...
    if ( ! empty($pickup_point) ) {
      echo '<h2>' . esc_attr__('Pickup point', WC_PAKETTIKAUPPA_TEXT_DOMAIN) . '</h2>';
  ...
  public function validate_checkout_pickup_point() {
    ...
    if ( $_POST['pakettikauppa_pickup_point'] === '__NULL__' ) {
      wc_add_notice(__('Please choose a pickup point.', WC_PAKETTIKAUPPA_TEXT_DOMAIN), 'error');
    }
  }
}
