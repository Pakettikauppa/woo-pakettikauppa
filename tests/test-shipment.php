    ...
    public function test_xml() {
		...
		foreach ($data['Parcels'] as $parcel) {
            ...
            foreach ($parcel['Parcel.ContentLines'] as $content_line) {
				$add_ContentLine = new Pakettikauppa\Shipment\ContentLine();
                $add_ContentLine->setDescription($content_line['contentline.description']);
				$add_ContentLine->setQuantity($content_line['contentline.quantity']);
                $add_ContentLine->setCurrency($content_line['contentline.currency']);
                $add_ContentLine->setNetweight($content_line['contentline.netweight']);
                $add_ContentLine->setValue($content_line['contentline.value']);
                $add_ContentLine->setCountryOfOrigin($content_line['contentline.countryoforigin']);
                $add_ContentLine->setTariffCode($content_line['contentline.tariffcode']);

				$add_parcel->addContentLine($add_ContentLine);
            }
